...
Documents are synced alongside DAGs when [Git Sync](/features/git-sync) is enabled. They are tracked as the `doc` kind in the sync state. Files under `docs/` with the `.md` extension are included in sync scanning.
