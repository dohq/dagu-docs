...
- **Step override** (evaluated at runtime, can reference params/secrets/outputs):
  ...
  ```
- **Fallback**: if you set nothing, Boltbase uses `BOLTBASE_DEFAULT_SHELL`, then `$SHELL`, then `sh` on Unix; on Windows it prefers PowerShell, then `pwsh`, then `cmd.exe`.
...
- Without a shebang, the resolved shell runs the script file. When Boltbase provides the default Unix shell, it appends `-e` so the script stops on the first failing command (step-level shells are left unchanged).
...
- **Auto `-e` on POSIX shells:** When Boltbase supplies the default/DAG-level shell for sh/bash/zsh/ksh/ash/dash, it appends `-e` for both command strings and script runs. If you set a step-level shell, include `-e` yourself when desired.
...
- **nix-shell:** Boltbase defaults to `--pure` if you do not specify purity flags. When Boltbase supplies the shell, it also prepends `set -e;` to the command string unless you already provided it.
