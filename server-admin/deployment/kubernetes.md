...
The default `general` pool has `labels: {}`, which means it matches DAGs without a `worker_selector` (an empty selector matches any worker). See [Worker Labels](/server-admin/distributed/worker-labels#matching-algorithm) for how matching works.
...
These labels are then matched against `worker_selector` in DAG definitions. See [Distributed Execution — How Dispatch Decisions Work](/server-admin/distributed/#how-dispatch-decisions-work) for the full dispatch logic.
